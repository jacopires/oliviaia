  ...
  return (
    <div className="flex-1 overflow-y-auto bg-background-light dark:bg-background-dark p-6 md:p-8 lg:p-10 min-h-screen relative">
      ...
      <div className="absolute top-8 right-8 z-20">
        <Link to="/create-agent" className="hidden md:flex items-center gap-2 bg-primary hover:bg-primary/90 text-[#0a0f0d] font-semibold py-3 px-6 rounded-xl transition-all shadow-lg shadow-primary/10">
          <span className="material-symbols-outlined text-xl font-light">add</span>
          <span className="text-sm tracking-tight">Novo Agente</span>
        </Link>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-white dark:bg-surface-dark border border-gray-200 dark:border-white/5 rounded-2xl p-6 relative overflow-hidden group hover:border-emerald-500/30 transition-colors">
              ...
              <div className="relative z-10 flex flex-col justify-between h-full gap-4">
                <div className="p-3 bg-emerald-500/5 text-emerald-500 w-fit rounded-lg border border-emerald-500/10">
                  <span className="material-symbols-outlined text-2xl font-light">filter_alt</span>
                </div>
                <div>
                  <p className="text-gray-500 font-medium text-[10px] uppercase tracking-[0.2em]">Total de Leads Hoje</p>
                  <h2 className="text-3xl font-semibold text-white/90 mt-1">42</h2>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-surface-dark border border-gray-200 dark:border-white/5 rounded-2xl p-6 relative overflow-hidden group hover:border-blue-500/30 transition-colors">
              ...
              <div className="relative z-10 flex flex-col justify-between h-full gap-4">
                <div className="p-3 bg-blue-500/5 text-blue-500 w-fit rounded-lg border border-blue-500/10">
                  <span className="material-symbols-outlined text-2xl font-light">smart_toy</span>
                </div>
                <div>
                  <p className="text-gray-500 font-medium text-[10px] uppercase tracking-[0.2em]">Agentes Ativos</p>
                  <h2 className="text-3xl font-semibold text-white/90 mt-1">3<span className="text-lg text-gray-600 font-normal">/5</span></h2>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-surface-dark border border-gray-200 dark:border-white/5 rounded-2xl p-6 relative overflow-hidden group hover:border-yellow-500/30 transition-colors">
              ...
              <div className="relative z-10 flex flex-col justify-between h-full gap-4">
                <div className="p-3 bg-yellow-500/5 text-yellow-500 w-fit rounded-lg border border-yellow-500/10">
                  <span className="material-symbols-outlined text-2xl font-light">savings</span>
                </div>
                <div>
                  <p className="text-gray-500 font-medium text-[10px] uppercase tracking-[0.2em]">Economia Gerada</p>
                  <h2 className="text-3xl font-semibold text-white/90 mt-1">R$ 1.250</h2>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="flex flex-col lg:flex-row gap-6 h-auto lg:h-[500px]">
          ...
          <div className="w-full lg:w-2/3 bg-white dark:bg-surface-dark border border-gray-200 dark:border-white/5 rounded-2xl p-8 flex flex-col shadow-sm">
            ...
            <div className="flex-1 relative flex items-end justify-between gap-2 px-4 pb-4 border-b border-l border-gray-200 dark:border-white/5 border-dashed">
              ...
              {/* SVG Area Curve */}
              {/* SVG Area Curve */}
              <svg className="absolute inset-0 h-full w-full z-10" preserveAspectRatio="none" viewBox="0 0 1000 500">
                <defs>
                ...
                <div key={event.id} className="group flex gap-4 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-white/5 transition-colors border border-transparent dark:hover:border-white/5 cursor-default">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start mb-1">
                      <p className={`text-sm font-semibold truncate ${event.type === 'success' ? 'text-emerald-400' :
                        event.type === 'warning' ? 'text-yellow-400' :
                          event.type === 'info' ? 'text-blue-400' :
                            'text-red-400'
                        }`}>{event.title}</p>
                      <span className="text-[10px] text-gray-600 font-mono whitespace-nowrap ml-2">{event.time}</span>
                    </div>
